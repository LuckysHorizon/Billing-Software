-- Reset passwords for Grocery POS users
USE grocery_pos;

-- Update admin user with simple password
UPDATE users SET password_hash = 'admin123' WHERE username = 'admin';

-- Update cashier user with simple password  
UPDATE users SET password_hash = 'cashier123' WHERE username = 'cashier';

-- Verify the updates
SELECT username, password_hash, full_name, role FROM users WHERE username IN ('admin', 'cashier');
